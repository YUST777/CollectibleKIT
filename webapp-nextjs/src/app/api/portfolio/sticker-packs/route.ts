...
export async function GET(request: NextRequest) {
  try {
    ...
    const collectionSnakeCase = collectionName.toLowerCase().replace(/\s+/g, '_');
    
    // Get packs from the static data
    const packsData = STICKER_COLLECTIONS[collectionSnakeCase];
    ...
    // Convert pack names to Title Case with filename info
    const packs = Object.entries(packsData)
      .map(([packName, filename]) => {
        const words = packName.split('_');
        const displayName = words.map(word => 
          word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()
        ).join(' ');
        return { name: displayName, filename };
      })
    ...
    return NextResponse.json({ 
      success: false, 
      error: 'Failed to fetch sticker packs' 
    }, { status: 500 });
  }
}
