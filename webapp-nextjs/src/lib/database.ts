...
class DatabaseService {
  ...
  async getPortfolioHistory(userId: number): Promise<any[]> {
    try {
      const rows = await this.dbAll(
        `SELECT 
          date,
  ...
  async savePortfolioSnapshot(userId: number, totalValue: number, giftsCount: number): Promise<void> {
    try {
      ...
      const existing = await this.dbAll(
      ...
      if (existing && existing.length > 0) {
        ...
        await this.dbRun(
          `UPDATE portfolio_history SET total_value = ?, gifts_count = ?, created_at = ? WHERE user_id = ? AND date = ?`,
          [totalValue, giftsCount, timestamp, userId, today]
        );
      } else {
        // Insert new snapshot
        await this.dbRun(
          `INSERT INTO portfolio_history (user_id, date, total_value, gifts_count, created_at) VALUES (?, ?, ?, ?, ?)`,
          [userId, today, totalValue, giftsCount, timestamp]
        );
      }
    } catch (error) {
