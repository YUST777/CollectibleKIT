...
import { Share2, Link2, ChevronLeft, ChevronRight, Code, Zap } from 'lucide-react';
                ...
                <div className="absolute bottom-2 md:bottom-4 left-3 right-3 md:left-5 md:right-5 flex items-center justify-between z-30">
                    ...
                    <div className="flex gap-2">
                        <button
                            onClick={() => {
                                if (navigator.share) {
                                    navigator.share({ title: `${data.username}'s 2025 Recap`, text: `I solved ${data.totalSolved} problems on ICPC HUE!`, url: window.location.href }).catch(() => { });
                                } else {
                                    navigator.clipboard.writeText(window.location.href);
                                    alert('Link copied!');
                                }
                            }}
                            className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-white/15 transition-all"
                        >
                        ...
                        <button className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-white/15 transition-all">
